...
def abs_no_resolve(p) -> Path:
    """dsh-patch: absolutise WITHOUT resolving links/junctions.

    Path.resolve() follows the ~/dsh-ghidra-workspace junction back to
    ~/.dsh/..., whose '.dsh' element Ghidra's ProjectLocator rejects outright.
    os.path.abspath normalises the path but does not chase reparse points, and
    keeps hashes/endpoints deterministic for a given spelling.
    """
...
def socket_path_for_project(gpr: Path) -> Path:
    ...
    """
    digest = hashlib.sha256(str(abs_no_resolve(gpr)).encode()).hexdigest()[:8]
...
def session_file_path(gpr: Path) -> Path:
    ...
    return abs_no_resolve(gpr).parent / f".ghidra-rpc-{digest}.json"
...
def save(session: Session) -> None:
    ...
    data = {
        "mode": session.mode,
        "project_gpr": str(abs_no_resolve(session.project_gpr)),
...
def register(session: Session) -> None:
    ...
    path = _registry_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    digest = hashlib.sha256(str(abs_no_resolve(session.project_gpr)).encode()).hexdigest()[:8]
    entry = {
        "project_gpr":       str(abs_no_resolve(session.project_gpr)),
        "socket_path":       str(session.socket_path),
        "mode":              session.mode,
