...
class HeadlessContext:
    ...
    def __init__(self, session: Session):
        ...
        # dsh-patch: do not resolve() the project dir; the dsh workspace lives
        # behind a junction whose target contains a '.dsh' element that
        # ProjectLocator rejects. abs_no_resolve keeps the junction spelling.
        from ghidra_rpc.session import abs_no_resolve as _abs_nr
        locator = ProjectLocator(str(_abs_nr(project_dir)), project_name)
        if locator.exists():
            self.project = GhidraProject.openProject(
                str(_abs_nr(project_dir)), project_name, True
            )
        else:
            self.project = GhidraProject.createProject(
                str(_abs_nr(project_dir)), project_name, False
            )
