...
字段偏移速查（相对 IPv4 头；`--offset` **只读 1 字节**）：TOS=1，TTL=8，TCP flags=IP 头长+13。

> **IPID 陷阱**：IPID 是 **16 位大端**，高字节在 IP+4、低字节在 IP+5。
> 只写 `IPID=4` 会读到恒为 0 的高字节（直方图全 0）—— 值 < 256 的隐信道**必须用 `--offset 5`**。
> 实测：把 12 字节 ASCII 依次写进 IPID 低字节，`--offset 4` 得到 12 个 0，`--offset 5` 才还原出原文。

## 载体判定 tells（pcap+二进制复合题）

1. 某字段是下标的纯函数（含公式化，如 `payload=(7-3·rank) mod 16` 零偏差）⇒ 载体/诱饵，不是数据
2. 同逻辑键（重传/同下标）字段取值冲突 ⇒ 复合值候选（明文是 ≥2 字段函数）；真实重传载荷可合法不同——假设生成器，非排除器
3. 二进制里零引用数据表 ⇒ 置换/替换/密钥材料（ghidra-core `unreferenced_data.py` 分类）
4. 载体位数 = 目标长度 ⇒ 信道判对的强证据（仍可作诱饵）
5. 存在刻意重复包 ⇒ 出题人给的一致性 oracle——先用 `conflict_oracle.py` 消费它
...
- **N 个包只产出 N−1 个 bit**（脚本取的是相邻命中包的差，`interval[i]=t[i+1]-t[i]`）。
  所以编码 M 个 bit 需要 **M+1 个包**；解出的位数比预期少 1 时，**优先怀疑尾部缺包**
  （补一个尾部包重跑），再考虑在开头补 0 —— 二者都会「差 1 bit」，但成因相反。
  实测：32 bit 的 `TICK` 用 32 个包只解出 31 bit（`TIC`），补一个尾包后 32/32 正确。
