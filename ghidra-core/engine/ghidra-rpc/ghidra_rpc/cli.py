...
def _resolve_project(project: str | None) -> Path:
    ...
    # dsh-patch: absolutise without following junctions (see session.abs_no_resolve)
    if project:
        return session_mod.abs_no_resolve(project)
    env = os.environ.get("GHIDRA_RPC_PROJECT")
    if env:
        return session_mod.abs_no_resolve(env)
    click.echo(
...
# ── dsh custom commands (dsh-patch) ──────────────────────────────────────────


@cli.command(name="exec-code")
@click.argument("binary")
@click.argument("code_file")
@click.option("--project", "-p", type=str, help="Path to .gpr project file")
def exec_code_cmd(binary: str, code_file: str, project: str | None):
    """dsh: execute a Python file inside the daemon with full Ghidra API.

    Predefined names: program, currentProgram, fm, listing, memory, toAddr,
    monitor, find_function, output_json. UNSANDBOXED by design.
    """
    _rpc_command(_resolve_project(project), "exec_code", {
        "binary": binary, "code_file": code_file,
    })
...
@click.argument("binary")
@click.option("--project", "-p", type=str, help="Path to .gpr project file")
def triage_cmd(binary: str, project: str | None):
    """dsh: one-shot triage report (metadata/imports/suspicious APIs/strings/lang hints)."""
    _rpc_command(_resolve_project(project), "triage", {"binary": binary})


@cli.command(name="export-binary")
@click.argument("binary")
@click.argument("path")
@click.option("--project", "-p", type=str, help="Path to .gpr project file")
def export_binary_cmd(binary: str, path: str, project: str | None):
    """dsh: export the program to its original file format (with md5 comparison)."""
    _rpc_command(_resolve_project(project), "export_binary", {
        "binary": binary, "path": path,
    })


@cli.command(name="emulate-function")
@click.argument("binary")
@click.argument("func")
@click.option("--registers", "registers_json", default="{}",
              help="JSON object of register inputs, e.g. '{\"RAX\":\"0x1234\"}'")
@click.option("--memory", "memory_json", default="{}",
              help="JSON object of memory inputs keyed by address")
@click.option("--max-steps", "max_steps", type=int, default=10000,
              help="Maximum instructions to execute (default 10000)")
@click.option("--project", "-p", type=str, help="Path to .gpr project file")
def emulate_function_cmd(binary: str, func: str, registers_json: str,
                         memory_json: str, max_steps: int, project: str | None):
    """dsh: emulate a function with EmulatorHelper (P-code)."""
    try:
        registers = json.loads(registers_json) if registers_json else {}
        memory_inputs = json.loads(memory_json) if memory_json else {}
    except json.JSONDecodeError as e:
        _json_error("ValueError", f"invalid JSON in --registers/--memory: {e}")
        return
    _rpc_command(_resolve_project(project), "emulate_function", {
        "binary": binary, "function": func,
        "registers": registers, "memory": memory_inputs,
        "max_steps": max_steps,
    })
