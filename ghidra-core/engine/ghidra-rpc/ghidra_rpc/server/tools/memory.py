...
def _handle_write_bytes(ctx, args: dict) -> dict:
    ...
    def do_write():
        ...
        # dsh-patch: align with the skill's patch_bytes.py — a code block is
        # normally flagged non-writable, which a binary patch has to overcome
        # (that flag is a permission, not a protection). Grant it for the
        # duration of the write and restore it afterwards, and clear the code
        # units covering the patch range so Ghidra does not reject the change
        # with "Memory change conflicts with instruction at ...".
        end_addr = addr.add(len(data) - 1)
        block = memory.getBlock(addr)
        granted_write = bool(block is not None and not block.isWrite())
        cleared_units = 0
        ...
        with ghidra_transaction(
            ...
            try:
                ...
                for cu in occupied:
                    ...
                    truncated_end = end if end.compareTo(end_addr) > 0 else end_addr
        ...
        return {
            ...
            "hex":      data.hex(),
            "verified": verified,
            "instructions_cleared": cleared_units,       # dsh-patch
            "write_permission_granted": granted_write,   # dsh-patch
        }
