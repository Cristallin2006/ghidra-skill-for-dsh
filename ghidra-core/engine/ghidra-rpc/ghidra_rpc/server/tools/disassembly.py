    ...
    def do_assemble():
        ...
        with ghidra_transaction(
            ...
            try:
                ...
                error_msg = str(e)
                # dsh-patch: SLEIGH mnemonics are case-sensitive ("nop" is
                # rejected where "NOP" assembles). Retry once with the text
                # upper-cased before surfacing the failure.
                if "Syntax Error" in error_msg or "AssemblySyntaxException" in error_msg:
                    upper_lines = [line.upper() for line in instr_lines]
                    if upper_lines != list(instr_lines):
                        try:
                            result_block = asm.assemble(addr, *upper_lines)
                        except Exception:
                            raise ValueError(
