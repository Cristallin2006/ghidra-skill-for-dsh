...
| `emulate_blob.py` | 裸 blob Unicorn 仿真骨架：`--base/--entry/--rsp/--max-insns/--reg/--map-file/--dump-dir`（`--entry` 是相对 `--base` 的偏移），停止原因分类 + dirty 页 dump；**入口未映射判 harness 配置错 exit 3**（不误报 RET 飞出）；unicorn 缺失 exit 3 提示进 re-tools-venv（多层载荷题用，ctf-patterns §10） |
