...
把"读伪码"从靠手感变成可机械把关的工程流程。适用范围：一切打算**引用伪码内容下结论**的场景；纯导航不受本文约束。实测基线（happyVm 样本，468 函数 / 2.0 MB 伪码）：**6% 的函数伪码不可直接引用、255 处常量渲染可疑**——撞上陷阱不是运气差，是系统性问题，必须系统化处理。
...
**① 体检**（先做，别跳）：`decompile-all` → `decomp_lint.py` → 拿 fatal 函数清单 + 危险渲染计数。**fatal 清单内的 6% 函数禁止读伪码**，直接走 L1/L2。
